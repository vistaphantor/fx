...
def test_generic_prompt_does_not_duplicate_human_prefix():
    parsed = _parse_source_example(
        {
            "prompt": "Human: What is 2 + 2?",
            "response": "Assistant: 4",
        },
        source="test",
    )
    ...
    assert "Assistant: Assistant:" not in text
    assert "<user>\nWhat is 2 + 2?\n</user>" in text
    assert "<assistant>\n4\n</assistant>" in text
...
def test_combined_prompt_does_not_leak_answer_into_user_turn():
    ...
    assert "Assistant:" not in user_section
...
def test_kimi_conversations_inside_embedded_prompt_are_recovered():
    ...
    assert parsed.messages[0] == TrainingMessage(
        "user",
        "Solve 2 + 2.",
    )
...
def test_teichai_serialized_message_lines_are_recovered():
    ...
    assert "<user>\nWhat is 2 + 2?\n</user>" in text
    assert "<think>Calculate.</think> 4" in text
...
def test_reasoning_tokens_are_canonicalized_to_boundaries():
    parsed = _parse_source_example(
        {
            ...
            "response": (
                "<think>Calculate carefully.</think>4"
            ),
        },
        source="test",
    )
    ...
    assert "carefully.</think>" not in serialized


def test_structural_tokens_do_not_depend_on_adjacency():
    example = TrainingExample(
        messages=(
            TrainingMessage(
                "assistant",
