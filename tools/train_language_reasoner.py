...
def _exam_payload(result: EpochExamResult | None) -> dict | None:
    if result is None:
        return None
    return {
        "epoch": result.epoch,
        ...
        "correctness_percent": result.correctness_percent,
        "mean_quality_percent": result.mean_quality_percent,
        "gibberish_answers": result.gibberish_answers,
    }
...
def main() -> None:
    ...
    if args.resume:
        ...
        last_train_loss = state.get("last_training_loss")
        last_val_loss = state.get("last_validation_loss")
    ...
    if start_epoch == 0:
        previous_exam = _run_and_save_exam(
            model=model,
            tokenizer=tokenizer,
            epoch=0,
            stage=stage,
    ...
    elif not (exams_dir / f"epoch_{start_epoch:03d}_exam.json").exists():
        previous_exam = _run_and_save_exam(
            model=model,
            tokenizer=tokenizer,
            epoch=start_epoch,
    ...
    for epoch in range(start_epoch, cfg["epochs"]):
        ...
        state_payload = {
            **contract,
            ...
            "last_exam": _exam_payload(previous_exam),
        }
        _atomic_torch_save(state_payload, state_path)

        current_exam = _run_and_save_exam(
            model=model,
            tokenizer=tokenizer,
            epoch=epoch + 1,
            stage=stage,
    ...
    metrics = {
        ...
        "epoch_exam_required": True,
        "epoch_exam_version": 1,
        "last_epoch_exam": _exam_payload(previous_exam),
        **budget,
    }
    save_model_bundle(
