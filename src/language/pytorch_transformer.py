...
class TransformerBlock(nn.Module):
    ...
    def forward_cached(
        ...
        """Inference-only attention with reusable keys/values.

        The initial prompt may contain many tokens and therefore uses causal
        masking. Cached continuation calls contain exactly one new token, whose
        query is allowed to attend every cached key plus itself.
        """
        batch, time_steps, channels = x.shape
        q, k, v = self._qkv(x)
        if past is None:
