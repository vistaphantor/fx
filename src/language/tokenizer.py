...
class BPETokenizer:
    ...
    def train(self, text: str, vocab_size: int = 8192, min_frequency: int = 2) -> None:
        print(f"[Tokenizer] Training byte BPE v4 on {len(text):,} chars, target vocab={vocab_size}...", flush=True)
        ...
        # Learn BPE over bounded lexical/whitespace segments. Exact whitespace
        # remains represented as bytes; segmentation only controls merge-search
        # cost and never changes the encoded byte stream.
        segment_freqs: Counter[str] = Counter()
        for part in ordinary_parts:
            segment_freqs.update(re.findall(r"\s+|\S+", part))
        ...
        for step in range(n_merges):
            ...
            for segment, syms in list(splits.items()):
                out: list[str] = []
                i = 0
                while i < len(syms):
            ...
            if (step + 1) % 500 == 0:
                ...
                print(
                    f"  [BPE] merges={step + 1:,}/{n_merges:,} vocab={len(vocab):,} "
                    f"pairs={len(pair_freqs):,} elapsed={elapsed:.1f}s",
                    flush=True,
                )
    ...
    def encode(self, text: str, add_bos: bool = True, add_eos: bool = True) -> list[int]:
        ...
        for part in re.split(pattern, text):
            ...
            if part in SPECIAL_TOKENS:
                ...
                # Preserve exact byte order while keeping the cache bounded to
                # lexical/whitespace segments rather than entire documents.
    ...
    def decode(
        ...
        *,
        ...
        `strict` is the authoritative round-trip mode and raises if an arbitrary
        token sequence is not valid UTF-8. Autoregressive generation is allowed
        to produce incomplete/malformed byte sequences before the model learns
        UTF-8 structure, so exam/chat callers may explicitly request
        `errors="replace"` without weakening tokenizer integrity checks.
        """
        if errors not in {"strict", "replace", "ignore"}:
            raise ValueError(f"unsupported_decode_error_policy:{errors}")
        ...
        def flush_bytes() -> None:
            if byte_buffer:
                chunks.append(bytes(byte_buffer).decode("utf-8", errors=errors))
                byte_buffer.clear()
